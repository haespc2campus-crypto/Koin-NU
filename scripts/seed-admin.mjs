...
const [emailArg, passwordArg, nameArg = "Admin", usernameArg = "admin"] = process.argv.slice(2);
const email = String(emailArg || process.env.ADMIN_EMAIL || "").trim().toLowerCase();
const username = String(process.env.ADMIN_USERNAME || usernameArg || "admin").trim().toLowerCase();
...
try {
  ...
  const data = { ...(existing.rows[0]?.data || {}), id, username, email, password_hash: await hashPassword(password), full_name: nameArg, role: "admin", phone: "", status: "aktif", created_at: existing.rows[0]?.data?.created_at || new Date().toISOString() };

...
const testDataDir = path.join(root, ".tmp-smoke-data");
const testUploadsDir = path.join(root, ".tmp-smoke-uploads");
...
async function waitForServer(proc) {
  ...
  while (Date.now() < deadline) {
    try {
      const { response } = await request("/api/db");
      if (response.ok) return;
    } catch {}
    await new Promise((resolve) => setTimeout(resolve, 150));
  }
  throw new Error(`Server tidak siap. Log: ${proc.stderrText || proc.stdoutText || "-"}`);
}
